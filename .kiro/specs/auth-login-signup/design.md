...
**Purpose**: 이 기능은 Eluo Skill Hub 플랫폼에 이메일/패스워드 기반 인증 시스템을 제공한다. `@eluocnc.com` 도메인 사용자만 가입할 수 있도록 제한하며, 이메일 인증 코드 확인을 통해 신원을 검증한다. 회원가입 시 사용자 프로필을 `public.profiles` 테이블에 자동 저장하여 도메인 수준의 사용자 프로필 관리를 지원한다.

**Users**: Eluo CNC 소속 직원(스킬 소비자, 스킬 제작자)이 로그인/회원가입을 통해 플랫폼에 접근한다. 플랫폼 관리자는 인증된 사용자 기반으로 권한을 관리한다.

**Impact**: 기존 인증 없이 접근 가능한 상태에서 Supabase Auth 기반 세션 관리 체계로 전환한다. `@supabase/ssr` 패키지를 도입하여 기존 Supabase 클라이언트 구조를 SSR 호환으로 리팩터링한다. `public.profiles` 테이블과 PostgreSQL 트리거를 도입하여 인증 데이터와 분리된 사용자 프로필 영속성을 확보한다.
...
- 회원가입 시 `public.profiles` 테이블에 사용자 프로필 자동 저장
...
- 프로필 수정 UI는 포함하지 않는다 (도메인/인프라 계층의 수정 인터페이스만 제공)
...
- 기존 `Entity<T>` 기반 클래스와 `SkillRepository` 인터페이스 패턴을 따라 `UserProfile` 엔티티와 `UserRepository` 인터페이스를 설계한다
...
graph TB
    ...
    subgraph SupabaseCloud[Supabase]
        ...
        SupaDB[PostgreSQL - public.profiles]
        Trigger[DB Trigger - handle_new_user]
    end
    ...
    ServerClient --> SupaDB
    SupaAuth --> SupaEmail
    SupaAuth -->|INSERT auth.users| Trigger
    Trigger -->|INSERT public.profiles| SupaDB
...
- **Selected pattern**: Server Action 기반 인증. Next.js App Router의 Server Action에서 Supabase Auth API를 호출하여 서버 사이드 보안과 쿠키 기반 세션 관리를 통합한다. 프로필 생성은 PostgreSQL 트리거가 담당하여 애플리케이션 계층의 변경 없이 데이터 정합성을 보장한다.
- **Domain/feature boundaries**: `user-account` 바운디드 컨텍스트가 인증 도메인 로직(이메일 검증, 패스워드 정책)과 사용자 프로필 도메인(UserProfile 엔티티, UserRepository 인터페이스)을 소유한다. 인증 인프라(Supabase Auth 호출)와 프로필 영속성(Supabase DB 접근)은 인프라 계층에 캡슐화한다.
- **Existing patterns preserved**: DDD 3계층 분리, Entity/ValueObject 기반 클래스, `@/` 경로 별칭, Repository 인터페이스 패턴
- **New components rationale**: 미들웨어(세션 갱신), SSR 클라이언트 유틸리티(서버/브라우저 분리), `user-account` 바운디드 컨텍스트(도메인 격리), `public.profiles` 테이블(사용자 프로필 영속성), DB 트리거(프로필 자동 생성), `UserProfile` 엔티티(프로필 도메인 표현), `UserRepository`/`SupabaseUserRepository`(프로필 데이터 접근)
...
| Data / Storage | Supabase PostgreSQL (public.profiles 테이블) | 사용자 프로필 영속성, RLS 기반 접근 제어 | DB 트리거로 자동 생성 |
...
sequenceDiagram
    ...
    alt Success
        ...
        DB->>DB: INSERT public.profiles with id and email
        SR-->>UC: AuthResult success
    ...
    else Trigger failure
        DB-->>SB: Rollback auth.users INSERT
        SB-->>SR: AuthError
        SR-->>UC: AuthResult failure
        UC-->>SA: Error message
        SA-->>SP: Display error
...
| 6.1 | public.profiles 테이블 스키마 정의 | ProfilesTable (SQL) | - | - |
| 6.2 | DB 트리거를 통한 프로필 자동 생성 | handle_new_user (Trigger), ProfilesTable | - | Signup Flow |
| 6.3 | RLS 정책을 통한 프로필 접근 제어 | ProfilesTable (RLS Policies) | - | - |
| 6.4 | UserProfile 도메인 엔티티 정의 | UserProfile | - | - |
| 6.5 | UserRepository 인터페이스 정의 | UserRepository | UserRepository | - |
| 6.6 | SupabaseUserRepository 구현체 제공 | SupabaseUserRepository | UserRepository | - |
| 6.7 | 기존 SignUpUseCase 변경 없이 프로필 저장 보장 | handle_new_user (Trigger) | - | Signup Flow |
| 6.8 | 트리거 실패 시 트랜잭션 롤백으로 데이터 정합성 보장 | handle_new_user (Trigger) | - | Signup Flow |
...
| UserProfile | user-account/domain | 사용자 프로필 엔티티 | 6.4 | Entity (P2) | - |
| UserRepository | user-account/domain | 프로필 리포지토리 인터페이스 | 6.5 | - | Service |
...
| SupabaseUserRepository | user-account/infrastructure | Supabase 프로필 리포지토리 구현체 | 6.6 | Supabase Server Client (P0) | Service |
...
| ProfilesTable | Supabase SQL | 사용자 프로필 테이블 및 트리거 | 6.1, 6.2, 6.3 | auth.users (P0) | - |
...
| Field | Detail |
|-------|--------|
| Intent | 사용자 프로필을 표현하는 도메인 엔티티. `public.profiles` 테이블과 1:1 대응 |
| Requirements | 6.4 |

**Responsibilities & Constraints**
- `id`(UUID), `email`, `createdAt` 속성을 포함하는 사용자 프로필 데이터를 표현한다
- `id`는 `auth.users.id`와 동일한 값으로, 인증 사용자와 프로필의 1:1 관계를 보장한다
- 기존 프로젝트의 `Entity<T>` 기반 클래스를 상속하여 일관된 도메인 모델 패턴을 유지한다
- 도메인 엔티티이므로 외부 라이브러리를 직접 참조하지 않는다

**Dependencies**
- Outbound: Entity (shared/domain) -- 기반 클래스 (P2)
...
```
- Preconditions: `id`는 비어 있지 않은 UUID 문자열이다. `email`은 비어 있지 않은 문자열이다.
- Postconditions: 유효한 UserProfile 인스턴스가 생성된다
- Invariants: `id`는 생성 후 변경할 수 없다. `email`은 `@eluocnc.com` 도메인이다.

#### UserRepository

| Field | Detail |
|-------|--------|
| Intent | 사용자 프로필 데이터 접근을 위한 리포지토리 인터페이스 (의존성 역전) |
| Requirements | 6.5 |

**Responsibilities & Constraints**
- 도메인 계층에 위치하여 프로필 영속성의 인프라 의존성을 역전시킨다
- 프로필 조회(`findById`) 및 수정(`update`) 계약을 명세한다
...
```
- Preconditions: `id`는 비어 있지 않은 UUID 문자열이다
- Postconditions: `findById`는 존재하지 않는 프로필에 대해 `null`을 반환한다. `update`는 존재하는 프로필의 데이터를 갱신한다.
- Invariants: 도메인 계층에서 외부 라이브러리를 직접 참조하지 않는다
...
- 프로필 생성은 DB 트리거가 자동으로 처리하므로 이 유스케이스에서 프로필 관련 로직을 포함하지 않는다 (6.7)
...
| Field | Detail |
|-------|--------|
| Intent | UserRepository의 Supabase 기반 구현체. `public.profiles` 테이블에 대한 데이터 접근을 수행 |
| Requirements | 6.6 |

**Responsibilities & Constraints**
- UserRepository 인터페이스를 구현하여 `public.profiles` 테이블의 데이터를 조회 및 수정한다
- Supabase 클라이언트를 통해 RLS가 적용된 쿼리를 수행한다
- 프로필 생성은 DB 트리거가 담당하므로 INSERT 로직을 포함하지 않는다
- 조회 결과를 `UserProfile` 도메인 엔티티로 변환하여 반환한다

**Dependencies**
- Inbound: 향후 프로필 조회/수정 유스케이스 (application) -- 프로필 접근 (P1)
- External: @supabase/supabase-js SupabaseClient -- 데이터베이스 접근 (P0)
...
**Implementation Notes**
- Integration: `supabaseClient.from('profiles').select('*').eq('id', id).single()`을 사용하여 프로필을 조회한다. RLS 정책에 의해 인증된 사용자의 본인 프로필만 접근 가능하다.
- Validation: `findById`에서 조회 결과가 없으면 `null`을 반환한다. `update`에서 존재하지 않는 프로필을 수정하려 하면 에러를 발생시킨다.
- Risks: RLS 정책이 올바르게 설정되지 않으면 다른 사용자의 프로필에 접근할 수 있다. RLS 정책 검증 테스트를 반드시 수행한다.
...
인증 기능은 Supabase Auth가 사용자 인증 데이터와 세션을 관리하며, 사용자 프로필은 `public.profiles` 테이블에서 별도 관리한다. 도메인 모델은 값 객체, 결과 타입, 프로필 엔티티로 구성된다.

- **Entities**: `UserProfile` (사용자 프로필, `id`는 `auth.users.id` 참조)
...
- **Business Rules**:
  ...
  - 회원가입 시 프로필은 DB 트리거에 의해 자동 생성된다
  - 사용자는 본인의 프로필만 조회 및 수정할 수 있다

### Logical Data Model

Supabase Auth는 `auth.users` 테이블을 내부적으로 관리한다. 사용자 프로필은 `public.profiles` 테이블에서 관리하며, `auth.users`와 1:1 관계를 맺는다.
...
**public.profiles 테이블**:
- `id` (UUID, PK, FK -> auth.users.id ON DELETE CASCADE): 사용자 고유 식별자
...
`auth.users`에 새로운 레코드가 INSERT 될 때 `public.profiles`에 프로필을 자동 생성하는 트리거 함수이다. `SECURITY DEFINER`로 실행되어 `postgres` 역할의 권한으로 `public.profiles`에 접근한다. 트리거 함수가 실패하면 `auth.users` INSERT 트랜잭션도 롤백되어 데이터 정합성을 보장한다 (6.8).
...
**Key Decisions**:
- `SECURITY DEFINER`와 `SET search_path = ''`를 사용하여 보안 컨텍스트를 명시한다
- `AFTER INSERT` 트리거를 사용하여 `auth.users` INSERT가 완료된 후 프로필을 생성한다
- `ON DELETE CASCADE`를 사용하여 `auth.users`에서 사용자가 삭제되면 프로필도 자동 삭제된다

#### RLS (Row Level Security) 정책

```sql
-- 인증된 사용자가 본인의 프로필만 읽을 수 있다
CREATE POLICY "Users can view own profile"
  ON public.profiles FOR SELECT
  TO authenticated
  USING (auth.uid() = id);

-- 인증된 사용자가 본인의 프로필만 수정할 수 있다
...
**Key Decisions**:
- `TO authenticated` 역할을 지정하여 비인증 사용자의 접근을 원천 차단한다
- INSERT 정책은 정의하지 않는다. 프로필 생성은 `SECURITY DEFINER` 트리거가 담당하므로 RLS를 우회한다.
- DELETE 정책은 정의하지 않는다. 프로필 삭제는 `ON DELETE CASCADE`로 `auth.users` 삭제 시 자동 처리된다.
...
**DB Trigger Errors (프로필 생성 실패)**:
- 트리거 함수 내부 오류 발생 시 `auth.users` INSERT 트랜잭션이 롤백된다. Supabase Auth는 이를 회원가입 실패로 처리하여 `unknown_error`로 매핑된다.
...
- `UserProfile.create()`: 유효한 프로필 생성, 빈 id/email 거부 검증
- `SupabaseUserRepository`: `findById` 조회 성공/실패, `update` 수행 검증
...
### Integration Tests
- DB 트리거 검증: `auth.users` INSERT 시 `public.profiles`에 프로필이 자동 생성되는지 검증
- RLS 정책 검증: 인증된 사용자가 본인의 프로필만 SELECT/UPDATE 가능한지 검증
- RLS 정책 검증: 비인증 사용자의 프로필 접근이 차단되는지 검증
...
- **RLS 기반 데이터 접근 제어**: `public.profiles` 테이블에 RLS 정책을 적용하여 인증된 사용자가 본인의 프로필만 접근 가능하도록 한다
- **트리거 보안**: `handle_new_user` 함수는 `SECURITY DEFINER`로 실행되며 `search_path`를 비워 SQL 인젝션 경로를 차단한다
...
src/
  user-account/
    domain/
      ...
      AuthResult.ts
      AuthRepository.ts
      UserProfile.ts            (신규: 사용자 프로필 엔티티)
      UserRepository.ts         (신규: 프로필 리포지토리 인터페이스)
    application/
      LoginUseCase.ts
      SignUpUseCase.ts
    infrastructure/
      SupabaseAuthRepository.ts
      SupabaseUserRepository.ts (신규: 프로필 리포지토리 구현체)
  shared/
    infrastructure/
      supabase/
...
supabase/
  migrations/
    YYYYMMDDHHMMSS_create_profiles_table.sql  (신규: profiles 테이블, 트리거, RLS)
```
