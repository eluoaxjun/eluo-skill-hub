...
describe("useDashboardState", () => {
  ...
  describe("filteredSkills 파생 상태", () => {
    it('"전체" 선택 시 모든 스킬을 반환한다', () => {
      ...
      expect(result.current.filteredSkills).toHaveLength(mockSkills.length);
    });
    ...
    it("카테고리 선택 시 해당 카테고리의 스킬만 필터링한다", () => {
      ...
      const expectedCount = mockSkills.filter(
        (s) => s.category === "기획"
      ).length;
    ...
    it("검색어로 스킬명을 대소문자 무시 부분 일치 필터링한다", () => {
      const { result } = renderHook(() => useDashboardState());
    ...
    it("검색어로 스킬 설명을 부분 일치 필터링한다", () => {
      const { result } = renderHook(() => useDashboardState());

      act(() => {
        result.current.setSearchQuery("피그마");
      });

      // 피그마 관련 설명을 가진 스킬이 매칭되어야 한다
      expect(result.current.filteredSkills.length).toBeGreaterThan(0);
      expect(
        result.current.filteredSkills.some(
          (s) =>
            s.description.toLowerCase().includes("피그마") ||
            s.tags.some((tag) => tag.toLowerCase().includes("피그마"))
        )
      ).toBe(true);
    });

    it("검색어로 스킬 태그를 부분 일치 필터링한다", () => {
      const { result } = renderHook(() => useDashboardState());
      ...
      // "TypeScript" 태그를 가진 스킬이 매칭되어야 한다
      expect(result.current.filteredSkills.length).toBeGreaterThan(0);
      expect(
        result.current.filteredSkills.some((s) =>
          s.tags.some((tag) => tag.toLowerCase().includes("typescript"))
        )
      ).toBe(true);
    });
    ...
    it("카테고리와 검색어를 결합하여 필터링한다", () => {
      ...
      expect(
        result.current.filteredSkills.every(
          (s) =>
            s.name.toLowerCase().includes("api") ||
            s.description.toLowerCase().includes("api") ||
            s.tags.some((tag) => tag.toLowerCase().includes("api"))
        )
      ).toBe(true);
    });
  });
